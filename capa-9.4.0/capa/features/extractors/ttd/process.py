...
def get_threads(
    calls: dict[ProcessAddress, dict[ThreadAddress, list[TtdCall]]], ph: ProcessHandle
...
def extract_environ_strings(ph: ProcessHandle) -> Iterator[tuple[Feature, Address]]:
    """
    Extract strings from the process environment block.
    """
    for value in ph.inner.environ:
        if value:
