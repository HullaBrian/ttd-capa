...
def get_extractor(
    ...
    elif backend == BACKEND_TTD:
        import capa.features.extractors.ttd.extractor

        report = capa.helpers.load_json_from_path(input_path)
        return capa.features.extractors.ttd.extractor.TtdExtractor.from_report(report)
...
def get_file_extractors(input_file: Path, input_format: str) -> list[FeatureExtractor]:
    ...
    elif input_format == FORMAT_TTD:
        import capa.helpers
        import capa.features.extractors.ttd.extractor

        report = capa.helpers.load_json_from_path(input_file)
        file_extractors.append(capa.features.extractors.ttd.extractor.TtdExtractor.from_report(report))
